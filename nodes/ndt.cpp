...
NdtLocalizer::NdtLocalizer(ros::NodeHandle &nh, ros::NodeHandle &private_nh):nh_(nh), private_nh_(private_nh), tf2_listener_(tf2_buffer_){
  ...
  // Publishers
  sensor_aligned_pose_pub_ = nh_.advertise<sensor_msgs::PointCloud2>("points_aligned", 1000);
  ndt_pose_pub_ = nh_.advertise<geometry_msgs::PoseStamped>("ndt_pose", 1000);
  exe_time_pub_ = nh_.advertise<std_msgs::Float32>("exe_time_ms", 1000);
  transform_probability_pub_ = nh_.advertise<std_msgs::Float32>("transform_probability", 1000);
  iteration_num_pub_ = nh_.advertise<std_msgs::Float32>("iteration_num", 1000);
  diagnostics_pub_ = nh_.advertise<diagnostic_msgs::DiagnosticArray>("diagnostics", 1000);
  traj_publisher_ = nh_.advertise<nav_msgs::Path>("trajectory", 1000);

  // Subscribers
  down_size_sub_ = nh.subscribe("voxel_leaf_size", 1000, &NdtLocalizer::INIT_NDT, this);
  initial_pose_sub_ = nh_.subscribe("initialpose", 1000, &NdtLocalizer::callback_init_pose, this);
  map_points_sub_ = nh_.subscribe("points_map", 1, &NdtLocalizer::callback_pointsmap, this);
  sensor_points_sub_ = nh_.subscribe("filtered_points", 1000, &NdtLocalizer::callback_pointcloud, this);
...
void NdtLocalizer::INIT_NDT(const std_msgs::Float64::ConstPtr &msg){
	A_factor= msg->data;
}
  ...
  const sensor_msgs::PointCloud2::ConstPtr & sensor_points_sensorTF_msg_ptr)
{
  ...
  if (is_converged) {
    ...
    geometry_msgs::PoseStamped pose_msg;
		pose_msg.pose = result_pose_stamped_msg.pose;
		pose_msg.header = result_pose_stamped_msg.header;
